...
class UnifierWebService {
    ...
    async process(operation, data, options = {}, beforeSend) {
        var _a;
        try {
            const request = unifier_utils_1.runBeforeRequestHook({ data }, this.username, this.password);
            // console.log(request);
            ...
            const result = unifier_utils_1.runAfterResponseHook({ data: response, operationName: operation });
            return result;
        }
        catch (e) {
