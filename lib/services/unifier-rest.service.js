...
class UnifierRESTService {
                ...
                try {
                    ...
                    if (attachmentsList && attachmentsList.length > 0) {
                        const attachmentsWithData = await Promise.all(attachmentsList.map(async (attachment) => {
                            try {
                                const fileBuffer = await this.getBPAttachment(projectNumber, bpName, recordNo, attachment.id || attachment.attachment_id || attachment.fileName, options);
                                return {
    ...
    async getBPAttachmentsList(projectNumber, bpName, recordNo, options = {}) {
        try {
            ...
            const resp = await rest.get(`v1/bp/record/${projectNumber}/attachments?input=${encodeURIComponent(inputParam)}`);
            if (resp.data.status === 200) {
                return resp.data.data || [];
            }
            return [];
    ...
    async getBPAttachment(projectNumber, bpName, recordNo, attachmentId, options = {}) {
        try {
            ...
            const resp = await rest.get(`v1/bp/record/${projectNumber}/attachment?input=${encodeURIComponent(inputParam)}`);
